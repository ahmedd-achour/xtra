        ...
        Expanded(
          child: ElevatedButton(
            onPressed: () async {
              ...
              // Upload selectedImages to Firebase Storage
              for (var image in selectedImages) {
                ...
                Reference ref = FirebaseStorage.instance
                    ...
                    .child('offre_images')
                    .child(
                        '${FirebaseAuth.instance.currentUser!.uid}/$fileName.jpg');
              ...
              String delegation = await getStreet(pointsPicked);
              String governorat = await getGovernorat(pointsPicked);
              String pays = await getCountry(pointsPicked);

              // Call your createOfferFull function
              createOfferFull(
                time: Timestamp.fromDate(
                  DateTime.now().add(Duration(hours: 24)),
                ),
                categorie:
                    _selectedTags.contains(_tags[0]) ? _tags[0] : _tags[1],
                titre: _titleController.text,
                description: _description.text,
                prix: 0,
                utilisateurId: FirebaseAuth.instance.currentUser!.uid,
                pays: pays,
                gouvernorat: governorat,
                delegation: delegation,
                position: GeoPoint(
                  pointsPicked.lat.toDouble(),
                  pointsPicked.lng.toDouble(),
                ),
                images: imageUrls,
              );
              setState(() {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text("Offre uploaded successfully!"),
            ...
            child: isLoading
                ...
                : const Text("Deposer", style: TextStyle(color: Colors.white)),
          ),
        ),
      ],
    );
