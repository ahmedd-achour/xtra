...
// Function to fetch delegations (administrative divisions) from GeoNames API
  ...
  void _zoomToSelectedDelegation(String flag) {
    final country = countries.firstWhere(
      (element) => element['flag'] == flag,
      orElse: () => {'lat': '0.0', 'long': '0.0'}, // Default if not found
    );
            ...
            Positioned(
              ...
              child: Padding(
                ...
                child: GestureDetector(
                    onTap: () {
                      if (widget.isLogedIn) {
                        // drop down munue  first that contains the following : gerer profile : rotete to Profile scren gere offre that rotates for the same pagedeconection ,

                        Navigator.of(context).push(MaterialPageRoute(
                            builder: (context) => ProfileScreen()));
                      } else {
                        Navigator.of(context).push(MaterialPageRoute(
                            builder: (context) => Authentify()));
                      }
                    },
